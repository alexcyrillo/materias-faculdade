...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string local, int &qt);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
{
	int qt = 0;
	levantamentoAux(raiz, local, qt);
	return qt;
}

void avl::levantamentoAux(noh* umNoh, string local, int &qt)
{
	if(umNoh != NULL){
		levantamentoAux(umNoh->esq, local, qt);
		if(umNoh->elemento.localizacao == local){
			qt += umNoh->elemento.quantidadeDisponivel;
		}
		levantamentoAux(umNoh->dir, local, qt);
	}
}
