...
class avl {
	...
	private:
		...
		void levantamentoAux(noh* umNoh, string marca, short ano, int &qt);
	public:
		avl() { raiz = NULL; }
		~avl();
		void imprimir();
...
{
	int qt = 0;
	levantamentoAux(raiz, marca, ano, qt);
	return qt;
}

void avl::levantamentoAux(noh* umNoh, string marca, short ano, int &qt)
{
	if(umNoh != NULL){
		levantamentoAux(umNoh->esq, marca, ano, qt);
		if((umNoh->elemento.marca == marca) and (umNoh->elemento.anoFabricacao >= ano)){
			qt++;;
		}
		levantamentoAux(umNoh->dir, marca, ano, qt);
	}
}
