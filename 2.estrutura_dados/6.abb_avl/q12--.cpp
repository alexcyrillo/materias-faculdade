...
void avl::insere(const dado& umDado) {
	noh *nohAux = buscaAux(umDado.ano);
	if(nohAux != NULL)
...
noh* avl::insereAux(noh* umNoh, const dado& umDado) {
	if(umNoh == NULL){
		...
		if(umDado.chave < umNoh->elemento.ano)
...
{
	levantamentoAux(raiz, *raiz->elemento.ano, *raiz->elemento.qtdadeFilmesApresentados);
	return;
}
...
{
	if(umNoh != NULL){
		levantamentoAux(umNoh->esq, ptrAnoProcurado, ptrMaiorNumFilmesApres);
		if(umNoh->elemento.qtdadeFilmesApresentados >= ptrMaiorNumFilmesApres){
			ptrMaiorNumFilmesApres = umNoh->elemento.ano;
			if(umNoh->elemento.ano == )
		}
		levantamentoAux(umNoh->dir, ptrAnoProcurado, ptrMaiorNumFilmesApres);
	}
}
