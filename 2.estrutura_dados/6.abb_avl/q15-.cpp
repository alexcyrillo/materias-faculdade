...
class avl {
	...
	private:
		...
		void levantamentoAux(noh* umNoh, string marca, int &qt);
	public:
		avl() { raiz = NULL; }
		~avl();
		void imprimir();
...
{
	int qt = 0;
	levantamentoAux(raiz, marca, qt);
	return qt;
}

void avl::levantamentoAux(noh* umNoh, string marca, int &qt)
{
	if(umNoh != NULL){
		levantamentoAux(umNoh->esq, marca, qt);
		if(umNoh->elemento.marca == marca){
			qt += umNoh->elemento.quantidadeDisponivel;
		}
		levantamentoAux(umNoh->dir, marca, qt);
	}
}
