...
void avl::imprimeAutorizadosAux(noh* atual, bool impar) {
    if(atual != NULL){
        ...
        if((!impar && verificaAutorizado(atual->elemento.chave, impar)) || (impar && verificaAutorizado(atual->elemento.chave, impar))){
...
bool avl::verificaAutorizado(tipoChave chave, int dia) {
    ...
    if(nohAux != NULL)
    {
        if(((nohAux->elemento.cpf % 2 == 0) && (dia % 2 == 0)) || ((nohAux->elemento.cpf % 2 == 1) && (dia % 2 == 1)))
            return 1;
        else 
            return 0;
    }
