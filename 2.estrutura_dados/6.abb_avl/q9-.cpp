...
class avl {
	...
	private:
		...
		void levantamentoAux(noh* umNoh, string especie, string  raca, int &qt);
	public:
		avl() { raiz = NULL; }
		~avl();
		void imprimir();
...
{
	int qt = 0;
	levantamentoAux(raiz, especie, raca, qt);
	return qt;
}

void avl::levantamentoAux(noh* umNoh, string especie, string  raca, int &qt)
{
	if(umNoh != NULL){
		levantamentoAux(umNoh->esq, especie, raca, qt);
		if((umNoh->elemento.especie == especie) and (umNoh->elemento.raca == raca)){
			qt++;;
		}
		levantamentoAux(umNoh->dir, especie, raca, qt);
	}
}
